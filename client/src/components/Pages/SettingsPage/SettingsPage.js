...
const SettingsPage = () => {
    ...
    const formSubmitHandler = (e) => {
        e.preventDefault();
        let userData = {};
        for(let key in inputData){userData[key] = inputData[key].value};
        
        let tempPasswordData = {};
        for(let key in passwordData){tempPasswordData[key] = passwordData[key].value}

        const params = {userData:{...userData}, passwordData:{...tempPasswordData}, id:_id, userType:userType, email:email}
        dispatch(updateUser(params));   
        setPasswordData({password:{name:'password', value:''},newPassword:{name:'newPassword', value:''}});
    }
    ...
    return (
        ...
        <input id="password" className="input" value={passwordData.password.value} type="password" placeholder={`Enter password`} name='password' onChange={e =>  changePasswordHandler(e)}/> 
        </div>
        <div className="inputContainer">
        <label className="label" for='password'>Enter New password</label>
        <input id="password" className="input" value={passwordData.newPassword.value} type="password" placeholder={`Enter New password`} name='newPassword'  onChange={e =>  changePasswordHandler(e)}/> 
        </div>
